...
def train(args):
    ...
    nb_tasks = len(variable['task_name'])
    acc_matrix = {
        "top1": np.zeros((nb_tasks, nb_tasks)),
        "mean": np.zeros((nb_tasks, nb_tasks)),
        "mix_top_mean": np.zeros((nb_tasks, nb_tasks)),
    }
    label_history = []
